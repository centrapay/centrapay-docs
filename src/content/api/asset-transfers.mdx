...
stateDiagram
    ...
    classDef final fill:#edebeb
    classDef inProgress fill:#fafafa

    subgraph Asset Transfer Lifecycle
    direction TB
    created:::inProgress --> failed:::final
    created --> claimed:::final
    created --> sent:::inProgress
    sent --> claimed
    sent --> cancelled:::final
    sent --> expired:::inProgress
    expired --> returned:::final
    end
    subgraph Legend
        direction LR
        start1[ ]:::inProgress - In Progress
        start2[ ]:::final - Final State
    end 
```