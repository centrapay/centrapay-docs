...
description: Farmlands Co-operative (Farmlands) have partnered with Centrapay Payment Gateway (Centrapay) to deliver a secure web portal for Farmlands Card payments.
nav:
...
5. **Add the Customer Reference (Optional)** to relate to the customer invoice.

    ![Enter the Customer's Farmlands Reference](/farmlands-payment-reference.png)

6. **Click Create**

7. **Check ID** - This response may be given for high-value or high-risk transactions. The user is required to request request the Cardholder to provide photo ID to check against the Cardholder’s name. The transaction remains in a “pending” state until the user confirms they have authenticated the Card.

    ![Check photo ID and accept condition on the Centrapay business portal](/farmlands-payment-check-photo-id.png)

8. **Approved** - Response received.

    ![Payment Authorization approved on the Centrapay business portal](/farmlands-payment-authorisation-approved.png)

9. **Copy Short Code** - Once approved this will bring up the Authorisation Approved screen, showing the “Short Code”. The Short code must be copied into the PO field of your invoice and then sent to Farmlands.
10. **Click Close** - This will redirect you to the Payment screen showing the authorisation history.
...
This area relates to the management of new and existing Branches/Locations used for Centrapay’s other payment types e.g tokens (i.e not Farmlands). Additional merchants added through the Merchants area will not be able to accept Farmlands Card until they are setup by Farmlands.
...
This function is only available to businesses who wish to use other Centrapay supported payment methods other than Farmlands Cards. (e.g. NZD).
...
1. An Account owner can **select the Members page** in the left sidebar menu.
![Centrapay business portal add members screen](/members/add-people.png)

2. The Account owner can click **Add people**.

3. **Enter the email address** of the new member.
![Enter Member's email address](/members/add-member-email.png)

4. **Select the member's role** (account owner or cashier).
![Select Member's role](/members/add-member-role.png)

5. Press **Send Invitation**.

6. The member that was added will receive an email that will allow them to accept the invitation and join the business account. The invitation will be in pending state until the member accepts the invitation.
![Invitation sent modal](/members/member-invited-modal.png)

7. User accepts invitation and is added to the list of members.
![User is added to Members list](/members/added-members-page.png)

**Revoke Invitation**

1. Account owner can **select the Members page** in the left sidebar menu.
![Select the Members page](/members/pending-members-page.png)

2. **Find the pending invite** they wish to revoke.

3. **Click on the three dots** beside the pending invite.
![Click on the three dots besides the invitation](/members/revoke-invitation.png)

4. Press **Revoke Invitation**.

5. The invitation is now revoked and will no longer show inside the pending invitation list.
![Invitation revoked modal](/members/invitation-revoked-modal.png)

**Removing members**

1. Account owner can **select the Members page** in the left sidebar menu.
![Select the Members page](/members/added-members-page.png)

2. Find the member they wish to remove.

3. Click on the three dots beside the member.
![Click on the three dots besides the member](/members/remove-member.png)

4. Press **Remove Member**.

5. Member is removed and will not longer show in the members list.
![Member removed modal](/members/member-removed-modal.png)
...
<AccordionListWrapper>
  <div>
    <Disclosure client:visible title="What do we do if a customer returns one or more items, and the invoice has already been sent to Farmlands?">
  ...
  <div>
    <Disclosure client:visible title="What do we do if the customer cancels their order after we have sent the invoice to Farmlands?">
      Raise a credit note as normal and insert the original authorisation short code on the invoice. This will enable us to identify the original invoice and customer account details.
    </Disclosure>
  </div>

  <div>
    <Disclosure client:visible title="What do we do if the customer cancels their order before we have sent the invoice to Farmlands?">
  ...
  <div>
    <Disclosure client:visible title="What do we do if we need to amend the value of the order prior to sending the invoice to Farmlands?">
  ...
  <div>
    <Disclosure client:visible title="How long are funds held for on a Customer’s Card?">
  ...
  <div>
    <Disclosure client:visible title="We received a message on the Portal stating the “Barcode is invalid.” What does this mean?">
  ...
  <div>
    <Disclosure client:visible title="How do I manage who has access to the Authorisation Portal?">
      User management and access is at the discretion of the Card Partner.
    </Disclosure>
  </div>

  <div>
    <Disclosure client:visible title="How do I release funds held on a customer’s Card?">
  ...
  <div>
    <Disclosure client:visible title="We have a regular customer. Do we need to obtain an authorisation?">
  ...
  <div>
    <Disclosure client:visible title="What if the barcode will not scan or is obscured?">
  ...
  <div>
    <Disclosure client:visible title="The authorisation request has prompted us to “Check ID.” The customer is not the actual Cardholder but one of the workers of the Cardholder. Can I accept the Card?">
      No, unfortunately you must only accept the Card if the true owner of the Card is making the purchase.
    </Disclosure>
  </div>

    <div>
    <Disclosure client:visible title="Can the Account-Owner add a new merchant/location to the Portal?">
