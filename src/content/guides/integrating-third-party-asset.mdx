...
To verify a request start by decripting the JWT.

**Decripted JWT Payload**
...
Use the decripted JWT validate the following fields:

|        Field        |                                                                         Assertion                                                                           |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| exp                 | Assert that exp is greater than the current Unix timestamp.                                                                                                 |
| aud                 | Assert that aud is equal to the base URL of your Uplink API.                                                                                                |
| request_body_sha256 | Hash the request payload using the [SHA256 algorythm](https://en.wikipedia.org/wiki/SHA-2). Assert that request_body_sha256 is equal to the hashed payload. |

If any assertion is not met the request must be rejected.
