...
| request_body_sha256 | A hash of the request payload, created using the [SHA256 algorythm](https://en.wikipedia.org/wiki/SHA-2). |

Use the decoded JWT fields to validate the following:

Expiry

Verify that the request authorisation has not expired to prevent against replay attack.

Exp is provided as a default expiry, with a 5 minute expiry window. Check that the default expiry has not passed by asserting that exp is greater than the current Unix timestamp.

To use a custom expiry window instead of the default provided in exp, add the duration of the expiry window in seconds to iat. To verify this custom expiry has not passed, assert that the value is greater than the current Unix timestamp.


Audience

To prevent against man-in-the-middle attacks assert that aud is equal to the base URL of your Uplink API.


Request payload

The request_body_sha256 property can be used to verify that the request payload has not been tampered with. Request_body_sha256 contains a hash of the request payload, created using the [SHA256 algorythm](https://en.wikipedia.org/wiki/SHA-2).
We can therefore assert the payload is unchanged by hashing the received request payload using the SHA256 algorythm and checking for equality with request_body_sha256.

**Example Of Verifying A Request Payload**
