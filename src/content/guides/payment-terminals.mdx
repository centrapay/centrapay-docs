...
nav:
  path: Reference/Centrapay Integrations
  order: 1
---

Integrating a merchant terminal with Centrapay APIs requires creating, cancelling, voiding and refunding [Payment Requests](https://docs.centrapay.com/api/payment-requests) on behalf of Merchants using a “merchant terminal” API key.
...
You can use this key to [Create an API Key](https://docs.centrapay.com/api/api-keys#create-an-api-key) with the “merchant terminal” role. A “merchant terminal” key has a role that can create, cancel, void and refund Payment Requests on behalf of merchants.

> You are responsible for the safety of your API keys

## Merchant Configs
Centrapay [Merchant Configs](https://docs.centrapay.com/api/merchant-configs) represent an available set of configured payment methods that can be utilized by one or more payment terminals by a Merchant.
