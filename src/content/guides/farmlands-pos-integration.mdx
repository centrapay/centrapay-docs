...
Funds requiring release can either be tracked internally or you can [list payment activities](https://docs.centrapay.com/api/payment-requests#list-payment-activities-for-a-payment-request) and sum the confirmation [payment activities](https://docs.centrapay.com/api/payment-requests#payment-activity).

> Releasing funds should only happen if there are remaining funds to release.
