...
4. [Polling the Payment Request](/api/payment-requests#get-a-payment-request) returns a list of pending `merchantConditions` in the response. The POS must prompt the cashier to [accept](/api/payment-requests#accept-payment-condition-for-a-payment-request-experimental) or [decline](/api/payment-requests#decline-payment-condition-for-a-payment-request-experimental) each [Payment Condition](#payment-conditions) in order to successfully complete a payment.
  ...
  <CodePanel title="Response">
    ...
    A successful payment is identified when all Payment Conditions have an `accepted` status and the [Payment Request](/api/payment-requests#payment-request) status is `paid`. Here, the POS must stop polling and display confirmation of the successful payment.
...
Each Card Partner site that needs to accept Farmlands Card payments must be set up as a separate Centrapay [Merchant](/api/merchants) with its own [Merchant Config](/api/merchant-configs). The Merchant Config contains a Farmlands [Payment Option](/api/merchant-configs#payment-option-config) that allows the site to accept Farmlands Payments.
...
- The POS MUST display a message to prompt the cashier to [accept](/api/payment-requests#accept-payment-condition-for-a-payment-request-experimental) or [decline](/api/payment-requests#decline-payment-condition-for-a-payment-request-experimental) any pending Payment Conditions.
- The POS MUST include full details for all [Line Items](/api/payment-requests#line-item) and an invoice number when creating a Payment Request where the purchase is being fulfilled immediately.
...
4. [Polling the Payment Request](/api/payment-requests#get-a-payment-request) returns a list of pending `merchantConditions` in the response. The POS must prompt the cashier to [accept](/api/payment-requests#accept-payment-condition-for-a-payment-request-experimental) or [decline](/api/payment-requests#decline-payment-condition-for-a-payment-request-experimental) each [Payment Condition](#payment-conditions) in order to successfully authorise a Pre Auth payment.
  ...
  <CodePanel title="Response">
    ...
    A successful authorisation is identified when all Payment Conditions have an `accepted` status, the [Payment Request](/api/payment-requests#payment-request) status is `paid` and the `preAuthStatus` is `authorized`. Here, the POS must stop polling and display confirmation of the successful authorisation.
...
6. The POS [releases any remaining authorised funds](/api/payment-requests#release-funds-held-for-a-pre-auth-payment-request-experimental) against the [Payment Request](/api/payment-requests#payment-request) back to the Cardholder.
To confirm if there are funds requiring release you can [list payment activities](/api/payment-requests#list-payment-activities-for-a-payment-request) and sum the confirmation [payment activities](/api/payment-requests#payment-activity).

> Only call release if there are funds requiring release.


  <CodePanel title="Request" method="POST" path="/api/payment-requests/{paymentRequestId}/release">
    ```bash
...
The POS must follow Centrapay’s guidelines on [handling errors](/guides/merchant-integration-error-handling) when dealing with inconsistencies in [Payment Request](/api/payment-requests#payment-request) statuses due to network issues or race conditions.
...
Cardholder purchases are refunded by [refunding the Payment Request](/api/payment-requests#refund).

When the Cardholder presents a transaction reference (e.g. a [Payment Request](/api/payment-requests#payment-request) short code or `externalRef`) on their paper receipt, the POS should use this reference to look up the Payment Request ID and initiate the refund.
...
1. The Cardholder presents a transaction reference on their paper receipt. The POS looks up the [Payment Request](/api/payment-requests#payment-request) ID using the transaction reference.
2. The POS [refunds the Payment Request](/api/payment-requests#refund) using its ID.

  <CodePanel title="Request" method="POST" path="/api/payment-requests/{paymentRequestId}/refund">
    ```bash
...
[Payment Requests](/api/payment-requests#payment-request) have a short code field that is human- and OCR-friendly. When combined with the date or merchant id, short codes can unambiguously identify the correct Payment Request.
...
> Some [Payment Activities](/api/payment-requests#payment-activity-experimental) are repeatable (e.g. Pre Auth Confirmations and Refunds) so these have their own unique short code in order to disambiguate them. The short codes for these activities are different from the original Payment Request short code.
...
- The POS MUST provide an invoice number and full details for all [Line Items](/api/payment-requests#line-item) when [creating a Payment Request](/api/payment-requests#create-a-payment-request) if the purchase is being fulfilled immediately.
...
- Full details for [Line Items](/api/payment-requests#line-item) MUST include the description, SKU, quantity, price, tax and the discount provided to the Cardholder.
...
- The total for all Line Item prices MUST sum to the [Payment Request](/api/payment-requests#payment-request) amount.
...
- [Payment Request](/api/payment-requests#payment-request) short codes MUST be included when completion is performed through existing reconciliation channels.
...
- Pre Auth Confirmations MUST include an invoice number and full details for all [Line Items](/api/payment-requests#line-item).
- Authorised funds that will not be confirmed MUST be [released](/api/payment-requests#release-funds-held-for-a-pre-auth-payment-request-experimental) in order to return the funds to the Cardholder (e.g. orders that are cancelled before fulfilment).
- Confirmed funds MUST be [refunded](/api/payment-requests#refund) in order to return them to the Cardholder. They cannot be released.

**Refund Requirements**

- [Refunds](/api/payment-requests#refund) can be partial or full. Partial refunds can be performed multiple times but the total refunded amount cannot exceed the paid amount.
...
- You MUST include an invoice number and full details for all [Line Items](/api/payment-requests#line-item).
