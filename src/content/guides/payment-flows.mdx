...
2. Customer device scans the QR code, fetches the [payment request details](/api/payment-requests#get-a-payment-request), and [completes payment](/api/legacy-payment-requests#requests-pay) as normal.
...
5. Merchant will have to call endpoints to [accept a Payment Condition](/api/payment-requests#accept-payment-condition) if there are [Payment Conditions](/api/payment-requests#payment-condition) on the Payment Request and the condition has an `awaiting-merchant` status.
