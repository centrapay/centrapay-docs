...
Below are our guidelines for dealing with inconsistencies in [Payment Request](/api/payment-requests#payment-request) statuses due to network issues or race conditions.
...
If the status of a transaction cannot be determined to be successful after retrying, then the Payment Request should be voided. [Voiding a Payment Request](/api/payment-requests#void) will cancel the request and trigger any refunds if necessary.
