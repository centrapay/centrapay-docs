...
customers to pay a partial amount of a payment request using Centrapay. This
allows Centrapay Payments to easily complement other methods of payment such as
cash. This also allows for reporting to track upselling from forms of payment
like [Token](https://docs.centrapay.com/api/assets#tokens-experimental).
For expample if a customer comes in and wants to purchase a loaf of b

Partial Payment is an extension to the Payment Request Protocol that allows
customers to pay a partial amount of a payment request using Centrapay.
However, it is the responsibility of the integrating POS to ensure payment
