...
- Set `value.amount` to the amount payable through Centrapay.
- Set `basketAmount` to the purchase total, including other forms of payment
(e.g., cash or card). If the customer intends to pay the entire outstanding
via Centrapay this should be equal to `value.amount`.
...
has been paid.
- If the remaining amount is greater than 0, then there is a remaining balance
outstanding you will need to request payment via other means. If the customer
cannot complete the transaction via other means then you should
