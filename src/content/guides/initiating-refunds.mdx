...
Refunds are initiated by calling the [Refund Payment Request](/api/payment-requests#refund-a-payment-request-experimental) endpoint.
...
Refund requests must provide an `externalRef` that is unique for all refunds made against the [Payment Request](/api/payment-requests#payment-request).
...
Refunds are initiated using the identifier for the [Payment Request](/api/payment-requests#payment-request). However, a patron is NOT expected to have this information available.
...
There are a number of reasons why refunds can fail. These are documented on the [Refund Payment Request](/api/payment-requests#refund) endpoint and should be communicated to the user.
...
[Payment Requests](/api/payment-requests#payment-request) may be paid by multiple assets. When refunding a multi-asset Payment Request, the refunded asset is not controllable by the merchant.
